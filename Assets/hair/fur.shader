	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 flowColor = tex2D(_FlowMap,i.uv);
				
				float2 flowdir = normalize(flowColor.rg*2-1);
				float2 flowuv = float2(i.uv.x * flowdir.x - i.uv.y * flowdir.y,i.uv.y *flowdir.x + i.uv.x * flowdir.y);
				float shiftTex = tex2D(_tSpecShift,flowuv).r;
